...
def build_local_synthetic_portfolio(req: dict[str, Any], target_run_id: str, reason: str) -> dict[str, Any]:
    """Build a deterministic query portfolio when Bodhi cannot run.

    Bodhi API-created UI-node runs can fail before HITL with runtime metadata
    errors. A refresh should still be able to proceed with a conservative,
    clearly labelled portfolio rather than blocking the whole pipeline.
    """
    brand = str(req.get("brand") or "Brand").strip() or "Brand"
    market = str(req.get("market") or "Market").strip() or "Market"
    domain = req.get("domain")
    topic_count = max(1, int(req.get("topic_count") or 8))
    queries_per_topic = max(1, int(req.get("queries_per_topic") or 6))
    query_limit = max(1, int(req.get("query_limit") or topic_count * queries_per_topic))
    seed_topics = [
        line.strip()
        for line in str(req.get("seed_topics") or "").splitlines()
        if line.strip()
    ]
    default_topics = [
        ("EV range, charging and battery confidence", "consideration", "EV model page"),
        ("Hybrid and alternative powertrain running costs and choice", "consideration", "Powertrain comparison page"),
        ("Family practicality, space and comfort", "consideration", "Interior and family practicality page"),
        ("Safety, reliability and trust", "trust_safety", "Safety and warranty page"),
        ("Value, offers, finance and total cost of ownership", "cost_value", "Finance and offers page"),
        ("Urban mobility and compact car fit", "consideration", "Compact model page"),
        ("Ownership, service and aftersales confidence", "ownership_loyalty", "Service and owner support page"),
        ("Local dealer, test drive and purchase support", "local_market", "Dealer and test-drive page"),
    ]
    if seed_topics:
        topic_defs = [(topic, "consideration", "Owned informational page") for topic in seed_topics]
    else:
        topic_defs = default_topics

    topics: list[dict[str, Any]] = []
    queries: list[dict[str, Any]] = []
    qid = 1
    for idx in range(topic_count):
        topic, journey, page_type = topic_defs[idx % len(topic_defs)]
        topic_id = f"t{idx + 1:03d}"
        topics.append({
            "topic_id": topic_id,
            "topic": topic,
            "journey_category": topic,
            "journey_stage": journey,
            "priority": "high" if idx < 5 else "medium",
            "recommended_page_types": [page_type],
            "why_this_topic_matters": "Fallback portfolio topic generated by Evidence Service when Bodhi portfolio workflow was unavailable.",
            "market_notes": f"Localise to {market}; verify claims against current owned and approved sources.",
        })
        templates = [
            f"What should {market} buyers know about {topic.lower()}?",
            f"How does {brand} help with {topic.lower()} in {market}?",
            f"Which trusted sources should buyers use to compare {topic.lower()}?",
            f"What are the practical buying criteria for {topic.lower()} in {market}?",
            f"How should shoppers compare {brand} with alternatives for {topic.lower()}?",
            f"What questions should owners ask about {topic.lower()} before purchase?",
        ]
        for j in range(queries_per_topic):
            if len(queries) >= query_limit:
                break
            query = templates[j % len(templates)]
            branded = brand.lower() in query.lower()
            queries.append({
                ...
                "journey_category": topic,
                "journey_stage": journey,
                "intent": "comparison" if "compare" in query.lower() else "commercial_research",
                "query_type": "branded" if branded else "non_branded",
                "answer_type": "comparison" if "compare" in query.lower() else "explainer",
                "commercial_value": "high" if j < 3 else "medium",
                "priority": "high" if j < 3 else "medium",
                "recommended_page_type": page_type,
                "suggested_owned_page_type": page_type,
                "reason_selected": "Evidence Service fallback query generated because Bodhi portfolio workflow failed before producing a portfolio.",
                "expected_ai_answer_source_types": [
                    "manufacturer site",
                    "automotive media/reviews",
                    "government/transport agencies",
                    "dealer sites",
                    "owner forums/communities",
                ],
                "market_localisation_notes": f"Use {market} context and avoid unverified prices, specifications, incentives or availability claims.",
            })
            qid += 1
        if len(queries) >= query_limit:
            break
    ...
    return {
        ...
        "portfolio_id": f"portfolio_{normalise_key(brand)}_{normalise_key(market)}_{now_epoch()}_{uuid.uuid4().hex[:6]}",
        "brand": brand,
        "market": market,
        "domain": domain,
        "language": req.get("language") or "English",
        "portfolio_source": "evidence_service_local_fallback",
        "deepresearch_status": "fallback_used",
        ...
        "metadata": {
            ...
            "requested_queries_per_topic": queries_per_topic,
        },
    }
...
def trigger_and_wait_for_portfolio(req: dict[str, Any], target_run_id: str) -> dict[str, Any]:
    ...
    inputs = {
        ...
        "domain": req.get("domain"),
        "owned_domains": "\n".join(req.get("owned_domains") or []),
        "brand_terms": "\n".join(req.get("brand_terms") or []),
        ...
        "planner_model": req.get("planner_model") or "gpt-5.2",
        "writer_model": req.get("writer_model") or "gpt-5.2",
        "search_api": req.get("search_api") or "google",
        "number_of_queries": req.get("number_of_queries") or 4,
        "max_search_depth": req.get("max_search_depth") or 2,
        "url_mode": req.get("url_mode") or "domain",
        "human_feedback": str(req.get("human_feedback") or "false").lower(),
    }
    ...
    try:
        ...
        if str(os.getenv("BODHI_PORTFOLIO_LOCAL_FALLBACK", "true")).lower() not in {"true", "1", "yes", "y", "on"}:
            raise
        portfolio = build_local_synthetic_portfolio(req, target_run_id, f"Bodhi portfolio trigger failed: {exc}")
        ...
        write_run_status(target_run_id, "running", {
            ...
            "portfolio_source": "evidence_service_local_fallback",
            "portfolio_fallback_reason": str(exc)[:500],
        })
        return stored
    ...
    write_run_status(target_run_id, "running", {"stage": "portfolio_generation_running", "portfolio_bodhi_run_id": bodhi_run_id, "portfolio_trigger_response": trigger})
    ...
    hitl_required = str(os.getenv("BODHI_PORTFOLIO_HITL_REQUIRED", "false")).lower() in {"true", "1", "yes", "y", "on"}
    hitl_result = None
    try:
        hitl_result = client.submit_first_ui_hitl(
            bodhi_run_id,
            inputs,
            timeout_seconds=int(os.getenv("BODHI_PORTFOLIO_HITL_TIMEOUT_SECONDS", os.getenv("BODHI_HITL_TIMEOUT_SECONDS", "45"))),
        ...
        if str(os.getenv("BODHI_PORTFOLIO_LOCAL_FALLBACK", "true")).lower() not in {"true", "1", "yes", "y", "on"}:
            raise
        portfolio = build_local_synthetic_portfolio(req, target_run_id, f"Bodhi portfolio HITL failed: {exc}")
        stored = store_portfolio(portfolio, req.get("brand", ""), req.get("market", ""), req.get("domain"))
        write_run_status(target_run_id, "running", {
            "stage": "portfolio_generation_completed",
            "query_portfolio_id": stored.get("portfolio_id"),
    ...
    if not hitl_result and str(os.getenv("BODHI_PORTFOLIO_LOCAL_FALLBACK", "true")).lower() in {"true", "1", "yes", "y", "on"}:
        portfolio = build_local_synthetic_portfolio(req, target_run_id, "Bodhi portfolio run did not expose a pending HITL/UI task before timeout.")
        stored = store_portfolio(portfolio, req.get("brand", ""), req.get("market", ""), req.get("domain"))
        write_run_status(target_run_id, "running", {
            "stage": "portfolio_generation_completed",
            "query_portfolio_id": stored.get("portfolio_id"),
            ...
            "portfolio_source": "evidence_service_local_fallback",
            "portfolio_bodhi_run_id": bodhi_run_id,
            "portfolio_fallback_reason": "No pending HITL/UI task found before timeout.",
        })
        return stored
...
def source_type_from_url(url: Any, owned_domains: set[str] | None = None) -> str:
    """Classify a URL by source type.

    When *owned_domains* is provided, any URL matching those domains is
    classified as ``owned_oem`` instead of relying on hardcoded brand names.
    """
    d = source_domain(url)
    u = str(url or "").lower()
    if not d:
        return "external_citation"
    # Check explicit owned domains first (multi-brand support)
    if owned_domains and any(d == od or d.endswith("." + od) for od in owned_domains):
        return "owned_oem"
    # Backward compatibility: known automotive OEM domains
    if any(x in d for x in ["nissan", "toyota", "honda", "mazda", "subaru", "mitsubishi", "tesla", "hyundai", "kia", "bmw", "mercedes", "volkswagen"]):
...
def run_phase2_refresh(job_id: str, req: dict[str, Any]) -> None:
    ...
    try:
        ...
        if portfolio_id:
            ...
            # Custom portfolio uploaded from frontend — skip Bodhi workflow entirely
            custom = req.get("custom_portfolio")
            if isinstance(custom, dict) and isinstance(custom.get("queries"), list) and custom.get("queries"):
                write_run_status(target_run_id, "running", {"stage": "custom_portfolio_processing", "source": "frontend_upload"})
                custom.setdefault("brand", req.get("brand", ""))
                custom.setdefault("market", req.get("market", ""))
                custom.setdefault("domain", req.get("domain"))
                custom["portfolio_source"] = "frontend_upload"
                portfolio = store_portfolio(custom, req.get("brand", ""), req.get("market", ""), req.get("domain"))
                portfolio_id = portfolio.get("portfolio_id")
                write_run_status(target_run_id, "running", {
                    "stage": "custom_portfolio_stored",
                    "query_portfolio_id": portfolio_id,
                ...
                raise RuntimeError("query_portfolio_mode is 'upload' but no valid custom_portfolio was provided. It must contain a non-empty 'queries' array.")
        elif mode == "synthetic":
